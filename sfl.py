...
def main(config=None, project="JAXUED_TEST"):
    ...
    for eval_step in range(config["num_updates"] // config["eval_freq"]):
        ...
        if eval_step % every == 0:
            ...
            print(f"поиск: решаемых {(p > 0).mean():.3f}, обучаемых "
                  f"{((p > 0) & (p < 1)).mean():.3f}, лучший p(1-p) {(p * (1 - p)).max():.3f}",
                  flush=True)
...
if __name__=="__main__":
    ...
    # === ACCEL ===
    group.add_argument("--sfl_num_levels", type=int, default=512,
                       help="сколько случайных уровней перебирать за фазу поиска (в статье 5000)")
    group.add_argument("--sfl_attempts", type=int, default=8,
                       help="сколько независимых эпизодов на уровень: p — это доля, "
                            "по одному эпизоду её не измерить")
    group.add_argument("--sfl_period", type=int, default=250,
