...
def main(config=None, project="JAXUED_TEST"):
    ...
    for eval_step in range(config["num_updates"] // config["eval_freq"]):
        ...
        print(f"DEBUG eval_step={eval_step} save_interval={config['checkpoint_save_interval']}", flush=True)
        if config["checkpoint_save_interval"] > 0:
            r = checkpoint_manager.save(eval_step, args=ocp.args.StandardSave(runner_state[1]))
            checkpoint_manager.wait_until_finished()
            print(f"DEBUG save returned {r}, latest={checkpoint_manager.latest_step()}", flush=True)
